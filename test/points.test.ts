...
describe('Points - add', () => {
  test('POST /add - Add points to payer', async () => {
    let res = await request(app).post('/add').send({
      payer: 'DANNON',
      points: 1000,
    ...
    res = await request(app).post('/add').send({
      payer: 'UNILEVER',
      points: 200,
    ...
    // no request body
    res = await request(app).post('/add');
...
describe('Points - rest (tested add first)', () => {
  test('POST /spend - Spend points and return points spent per payer', async () => {
    let res = await request(app).post('/spend').send({
      points: 5000,
    });
    expect(res.statusCode).toEqual(200);
    ...
    // overbalance
    res = await request(app).post('/spend').send({
      points: 1000000,
    });
    expect(res.statusCode).toEqual(400);
    ...
    // no request body
    res = await request(app).post('/spend');
    expect(res.statusCode).toEqual(400);
